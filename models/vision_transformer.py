...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, bias=True, drop=0., tuning_mode=None):
        ...
        if self.tuning_mode == 'cross_ssf':
            self.ssf1_0 = SSF(in_features)
        self.fc1 = nn.Linear(in_features, hidden_features, bias=bias[0])
        if self.tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf':
            self.ssf1 = SSF(hidden_features)

        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])

        if self.tuning_mode == 'cross_ssf':
            self.ssf2_0 = SSF(hidden_features)
        self.fc2 = nn.Linear(hidden_features, out_features, bias=bias[1])
        if self.tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf':
            self.ssf2 = SSF(out_features)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., tuning_mode=None):
        ...
        if self.tuning_mode == 'ssf' or tuning_mode == 'cross_ssf':
            self.ssf1 = SSF(dim * 3)
    ...
    def forward(self, x):
        ...
        if self.tuning_mode == 'cross_ssf':
            x = self.ssf1_0(x)
        if self.tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf':
            qkv = (self.ssf1(self.qkv(x))).reshape(
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=int(
            dim * mlp_ratio), act_layer=act_layer, drop=drop, tuning_mode=tuning_mode)
        self.ls2 = LayerScale(
            dim, init_values=init_values) if init_values else nn.Identity()
    ...
    def forward(self, x):
        if self.tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf':
            x = x + \
                self.drop_path1(self.ls1(
                    self.attn(self.ssf1(self.norm1(x)))))
            x = x + \
                self.drop_path2(
                    self.ls2(self.mlp(self.ssf2(self.norm2(x)))))
        else:
    ...
    def __init__(
        ...
        for _ in range(num_parallel):
            ...
            self.ffns.append(nn.Sequential(OrderedDict([
                ('norm', norm_layer(dim)),
                ('mlp', Mlp(dim, hidden_features=int(
                    dim * mlp_ratio), act_layer=act_layer, drop=drop)),
                ('ls', LayerScale(dim, init_values=init_values)
                 if init_values else nn.Identity()),
                ('drop_path', DropPath(drop_path)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True, tuning_mode=None):
        ...
        if self.tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf':
            self.ssf1 = SSF(embed_dim)

        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
        if (self.tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf') and norm_layer:
            self.ssf2 = SSF(embed_dim)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(torch.randn(
            1, num_patches + self.num_tokens, embed_dim) * .02)
        ...
        if tuning_mode == 'ssf' or self.tuning_mode == 'cross_ssf':
            self.ssf1 = SSF(self.num_features)
...
def ssf_reparam(net, args):
    prev_module = None
    # drop_rate = torch.tensor(args.drop_rate, device=args.device)
    drop_rate = args.drop_rate
    with torch.no_grad():
        for name, module in net.named_modules():

            # print(name,isinstance(prev_module, nn.Conv2d),isinstance(prev_module, nn.Linear),isinstance(module, SSF))
            if prev_module and (isinstance(prev_module, nn.Linear) or isinstance(prev_module, nn.Linear)) and isinstance(module, SSF):
                if isinstance(prev_module, nn.Linear):
                    scale = module.scale.unsqueeze(1)
                else:
                    scale = module.scale.unsqueeze(
                        -1).unsqueeze(-1).unsqueeze(-1)
                # 计算增量
                add_weight = prev_module.weight*scale - prev_module.weight.data
                add_bias = prev_module.bias*module.scale + module.shift - prev_module.bias.data
                # 加上增量
                if args.reparam_way == 'tanh':
                    # 过tanh增加非线性
                    prev_module.weight.data += F.tanh(add_weight)
                    prev_module.bias.data += F.tanh(add_bias)
                elif args.reparam_way == "leaky_relu":
                    leaky_relu = nn.LeakyReLU(0.1)
                    prev_module.weight.data += leaky_relu(add_weight)
                    prev_module.bias.data += leaky_relu(add_bias)
                elif args.reparam_way == 'drop':
                    # 类似dropout代码
                    weight_mask = (torch.rand(add_weight.shape)
                                   > drop_rate).float().to(args.device)
                    bias_mask = (torch.rand(add_bias.shape)
                                 > drop_rate).float().to(args.device)
                    prev_module.weight.data += weight_mask * add_weight
                    # / (1.0 - drop_rate)
                    prev_module.bias.data += bias_mask * add_bias
                    # / (1.0 - drop_rate)
                # 恢复SSF参数
                if args.reparam_init_method == 'constant':
                    nn.init.ones_(module.scale)
                    nn.init.zeros_(module.shift)
                elif args.reparam_init_method == 'normal':
                    nn.init.normal_(module.scale, mean=1, std=.02)
                    nn.init.normal_(module.shift, std=.02)

            # elif prev_module and  isinstance(prev_module, SSF) and (isinstance(module, nn.Linear) or isinstance(module, nn.Linear)) :
            #     if isinstance(module, nn.Linear):
            #         scale = prev_module.scale.unsqueeze(1)
            #     else:
            #         scale = prev_module.scale.unsqueeze(
            #             -1).unsqueeze(-1).unsqueeze(-1)
                
            #     add_weight = (module.weight.T*scale).T - module.weight.data
            #     # print("prev_module.shift",prev_module.shift.shape)
            #     # print("module.weight",module.weight.shape)
            #     add_bias = prev_module.shift @ module.weight.T
            #     # print("add_weight",add_weight.shape)
            #     # print("add_bias",add_bias.shape)
            #     # 加上增量
            #     if args.reparam_way == 'tanh':
            #         # 过tanh增加非线性
            #         module.weight.data += F.tanh(add_weight)
            #         module.bias.data += F.tanh(add_bias)
            #     elif args.reparam_way == "leaky_relu":
            #         leaky_relu = nn.LeakyReLU(0.1)
            #         module.weight.data += leaky_relu(add_weight)
            #         module.bias.data += leaky_relu(add_bias)
            #     elif args.reparam_way == 'drop':
            #         # 类似dropout代码
            #         weight_mask = (torch.rand(add_weight.shape)
            #                        > drop_rate).float().to(args.device)
            #         bias_mask = (torch.rand(add_bias.shape)
            #                      > drop_rate).float().to(args.device)
            #         module.weight.data += weight_mask * add_weight
            #         # / (1.0 - drop_rate)
            #         module.bias.data += bias_mask * add_bias
            #         # / (1.0 - drop_rate)
            #     # 恢复SSF参数
            #     if args.reparam_init_method == 'constant':
            #         nn.init.ones_(prev_module.scale)
            #         nn.init.zeros_(prev_module.shift)
            #     elif args.reparam_init_method == 'normal':
            #         nn.init.normal_(prev_module.scale, mean=1, std=.02)
            #         nn.init.normal_(prev_module.shift, std=.02)

            prev_module = module
        # print(f"reparam count:{count}")
