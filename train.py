...
import ipdb
from freeze_params import adjust_all_parameters
from freeze_params import adjust_partial_parameters
...
# Relaxed Param SSF
parser.add_argument('--use-relax-param', action='store_true', default=False,)
parser.add_argument('--reparam-interval', type=int, default=50, metavar='N',
                    help='how many batches to reparam')
parser.add_argument('--reparam-init-method', type=str, default='constant', 
                    help='reparam init method')
parser.add_argument('--reparam-way', type=str, default='tanh', 
                    help='reparam way')
parser.add_argument('--drop-rate', type=float, default=0.9, 
                    help='reparam way drop drop rate')
parser.add_argument('--use-full-warmup', action='store_true', default=False)
...
def main():
    ...
    if args.log_wandb:
        if has_wandb:
            ...
            _logger.FileHandler(args.output)
    ...
    if args.local_rank == 0:
        ...
        _logger.info(
            f"number of params for requires grad: {sum(p.numel() for p in model.parameters() if p.requires_grad)}")
    ...
    if args.distributed:
        if has_apex and use_amp == 'apex':
            ...
            model = NativeDDP(model, device_ids=[
                              args.local_rank], broadcast_buffers=not args.no_ddp_bb)
    ...
    if args.evaluate:
        ...
        if saver is not None:
            ...
            best_metric, best_epoch = saver.save_checkpoint(
                start_epoch, metric=save_metric)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.use_full_warmup:
                if epoch == 0:
                    adjust_all_parameters(args, model)
                if epoch == num_epochs//4:
                    adjust_partial_parameters(args, model)
            ...
            if args.local_rank == 0:
                ...
                if args.save_images and output_dir:
                    torchvision.utils.save_image(
                        ...
                        padding=0,
                        normalize=True)
        if args.use_relax_param and batch_idx % args.log_interval == 0:
            vision_transformer.ssf_reparam(model,args)
